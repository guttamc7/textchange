//Database class to access and put data to/from SQL database
...
public class DataBase{
  ...
  //Update the user name and phone-number for email
  public JSONObject updateUserInfo(String email, String name, String phoneNumber){
    JSONObject userInfoUpdated=new JSONObject(); 
    boolean infoUpdated=false; 
    
    try{
      PreparedStatement stat = con.prepareStatement("UPDATE User SET FullName='"+name+"', Phone='"+phoneNumber+"' WHERE User_ID='"+email+"'");
      stat.executeUpdate();
      infoUpdated=true;
    } 
    catch(SQLException e){
      System.out.println("Error while updating the userInfo from the mySql Database");
      e.printStackTrace();
    }finally{
      try{
        userInfoUpdated.put("userInfoUpdated",infoUpdated);
      }catch(JSONException e){
        System.out.println("Error while converting updateUserInfor data to JSON Object");
        e.printStackTrace();
      }
    }
  ...
  public int getNumLogins(String email){
    ...
    try{
      ...
      System.out.println("Error while converting the number of logins to integer from mySql Database");
      e.printStackTrace();
    }
    return numLogins;
  }
  ...
  //Get the course ID associated with the course in the database. If the course does not exist, -1 will be returned
  public int getCourseID(String course){
    ...
    try{
      ...
      System.out.println("The course does not exist in the database, so a -1 will be returned");
    }
    return courseID;
  }
  
  //Add a new textbook to the database. If a course is not specified by the user, the textbook is added with course id -1
  public JSONObject addTextbook(String sellerEmail,String title,String author,String isbn,String date,String price,String course,String image){
    JSONObject textbookAdded=new JSONObject();
    boolean added=false;
    int courseID=getCourseID(course);
    
    try{
      PreparedStatement stat = con.prepareStatement("INSERT INTO Ad (SellerEmail, Title, Author, ISBN, Date,Price,Course,Course_ID,Image) VALUES (?,?,?,?,?,?,?,?,?)");
  ...
  public JSONObject getTextbookData(String id){
    ...
    try{
      ...
      while(r.next()){
        ...
        textbookData.put("image",r.getString(10));
